...
export default function Blog() {
  ...
  return (
    <div
      id="blog_posts"
      className="blog-posts section panel lg:mx-2 overflow-hidden gap-3 bg-secondary dark:bg-white dark:bg-opacity-5 rounded-1-5 lg:rounded-2 swiper-parent"
    >
                    ...
                    {posts.map((post, index) => (
                      <SwiperSlide className="swiper-slide" key={index}>
                        <article className="post type-post panel overflow-hidden vstack gap-3">
                          <Link
                            className="position-absolute top-0 ltr:start-0 rtl:end-0 m-3 fs-7 fw-bold text-none z-1 bg-tertiary text-white py-narrow px-1"
                            href={`/blog`}
                          ...
                          <div className="panel vstack gap-1">
                            ...
                            <Link
                              href={`/blog-details/${post.id}`}
                              className="uc-link text-tertiary dark:text-secondary fs-7 xl:fs-6 fw-bold hstack gap-1 sm:mt-1 xl:mt-2"
                            >
                              <span>Read post</span>
